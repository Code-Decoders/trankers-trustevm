...
async function getContractData() {
  const web3 = new Web3("https://trustscan.one/api/eth-rpc");
  const logs = await fetch(
    `https://trustscan.one/api?module=logs&action=getLogs&fromBlock=0&toBlock=latest&address=${contractAddress[chainId].inventory}&topic0=0xc3d58168c5ae7397731d063d5bbf3d657854427343f4c083240f7aacaa2d0f62`
  ...
  for (let index = 0; index < logs.result.length; index++) {
    ...
    events.push(JSON.parse(JSON.stringify(decodedParameters, null, 4)));
  }
  console.log(events);
  return events;
}
